  ...
  async function goToMobileCheckout() {
    if (cart.length === 0) {
      await showSwal(
        'warning',
        'Belum ada produk',
        'Pilih produk terlebih dahulu sebelum lanjut.',
      );
      return;
    }
    ...
    <main className="min-h-screen bg-[#eef5e8] flex text-[#21351f]">
      ...
      <section
        className={`flex-1 flex-col min-w-0 ${
          mobileCheckoutOpen ? 'max-md:hidden' : 'flex'
        }`}
      >
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="font-bold text-[#21351f] max-md:text-lg max-md:leading-snug">
                      {product.name}
                    </p>

                    <p className="text-sm text-[#6f7b62] max-md:hidden">
                      Kode: {product.code || '-'}
                    </p>
                  ...
                  {cartItem ? (
                    <div className="hidden max-md:flex h-12 w-12 shrink-0 items-center justify-center rounded-xl border border-[#008f67] text-xl font-bold text-[#008f67]">
                      {cartItem.qty}
                    </div>
                  ) : (
      ...
      {!mobileCheckoutOpen && (
        <div className="hidden max-md:block fixed bottom-0 left-0 right-0 z-40 bg-white/95 px-4 py-4 shadow-[0_-10px_30px_rgba(0,0,0,0.10)]">
          <div className="flex gap-3">
            <button
              onClick={goToMobileCheckout}
              disabled={cart.length === 0}
              className="flex-1 rounded-full bg-[#00a878] px-6 py-4 text-white disabled:bg-gray-300 disabled:text-gray-500"
            >
            ...
            <button
              onClick={clearCart}
              className="flex h-[58px] w-[74px] items-center justify-center rounded-3xl border border-[#008f67] bg-[#eaffef] text-2xl text-[#008f67]"
            >
              ♙
            </button>
          </div>
          ...
          <header className="bg-white px-4 pt-6 pb-4 border-b border-[#dfe8d2]">
            <div className="flex items-center justify-between">
              <button
                ...
                className="h-11 w-11 rounded-full bg-[#2f4f32] text-white"
              >
                ‹
              </button>
              ...
              <div className="text-center">
                ...
                <p className="text-xs text-[#6f7b62]">
                  Hitung pesanan otomatis
                </p>
              </div>

              <button
                onClick={clearCart}
                className="h-11 w-11 rounded-full border border-red-200 text-red-500"
              >
                ×
              </button>
            </div>
    ...
    <div className={`flex h-full flex-col ${mobile ? 'gap-4' : ''}`}>
      ...
      <div
        className={`${
          mobile
            ? 'space-y-4'
            : 'flex-1 p-6 overflow-y-auto max-lg:max-h-[520px]'
        }`}
      >
        {cart.length === 0 && (
          <p className="text-center text-[#8a947d] mt-10">
            Belum ada item
          </p>
        )}
        ...
        {cart.map((item: any) => (
          <div
            key={item.id}
            className="rounded-3xl border border-[#eef2e8] bg-white p-4 shadow-sm"
          >
      ...
      <div
        className={`${
          mobile
            ? 'fixed bottom-0 left-0 right-0 z-50 rounded-t-3xl border-t border-[#dfe8d2] bg-white p-4 shadow-[0_-10px_30px_rgba(0,0,0,0.12)]'
            : 'border-t border-[#dfe8d2] p-6'
        }`}
      >
        <input
          type="number"
